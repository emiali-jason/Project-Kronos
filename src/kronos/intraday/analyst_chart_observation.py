...
PROTOCOL = (
    ...
    "context_sufficient describes sufficiency for VISIBLE CONTRADICTION checking, not independent pixel proof of candle completion. Use true only when readable date/time context and the full right edge let you check this Review window, stale/later evidence and distinguish the qualified region. KRONOS independently validates selected completed candles, exact endpoints and calendar/session authority from retained machine evidence. Static pixels need not prove that metadata. Use false/null for cropped/unreadable context, ambiguous qualified scope or any required contradiction check that cannot be performed; never upgrade those findings from machine expectations. later_evidence, forming_evidence and other_contradiction each use PRESENT/ABSENT/UNKNOWN. ABSENT is an affirmative visual finding supported by basis and readable labels, never a default for missing evidence. Later-day or completed future candles, stale qualified context, wrong temporal context or another known contradiction must be PRESENT. Unreadable dates/cropped edges and ambiguous scope remain UNKNOWN/insufficient, never PASS.",
    "A visible live/countdown/forming marker requires forming_evidence=PRESENT. If it contaminates qualified evidence set forming_excluded=false. It may be excluded only when the chart visibly and unambiguously separates that same-window current forming candle from the completed evidence used for every answer: forming_excluded=true, excluded_forming_date=its independently visible YYYY-MM-DD, and nonempty exclusion_basis naming the forming region and the completed evidence instead used. Do not infer this exclusion from the machine endpoint. Never exclude a later-day chart or completed future evidence. If forming evidence is ABSENT use forming_excluded=false, excluded_forming_date=null, exclusion_basis=null. If static pixels cannot prove completion, truthfully use forming_evidence=UNKNOWN with forming_excluded=null, excluded_forming_date=null and exclusion_basis=null. UNKNOWN alone does not block native compatibility when machine completion is independently validated and visible contradiction context is sufficient. Never force ABSENT or set context_sufficient=false solely for lack of invisible completion metadata. Missing/unreadable visual context still fails closed. International references cannot borrow native machine completion and remain supporting-only / NOT_INDEPENDENTLY_ESTABLISHED. Absence of a countdown alone does not prove completion. Optional completion describes the qualified candle, not an excluded forming candle.",

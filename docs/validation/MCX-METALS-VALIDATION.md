...
| CPR mapping | Verified: selected reference timeframe feeds prior completed H/L/C and all CPR/pivot outputs | Source contract records KGS Auto validation; no compile log stored here | KGS comparison was recorded in source comments; no comparison image stored here | Not required | KR-280 contains two version labels that need metadata reconciliation. |

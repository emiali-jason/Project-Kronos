...
| KR-260A | Market Data Adapter | Translates KR-251/KR-252 configuration outputs into a standardized symbol-routing contract. | `0.1.0` | `FOUNDATION` | Foundation | KR-251 model/execution outputs; KR-252 relationship outputs | Route readiness/status/reason, analysis symbol, execution text, primary/secondary/sector/parent symbols, and capability flags | Future KR-260 integration | Standardized symbol routing only. No `request.security()` calls and no KR-260 integration yet. |

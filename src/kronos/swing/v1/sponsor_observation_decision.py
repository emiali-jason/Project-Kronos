...
from kronos.swing.v1.kr370_step31_handoff import Kr370Step31EligibilityHandoffV2
...
SPONSOR_DECISION_SNAPSHOT_V2_CONTRACT_ID = "KRONOS-SWING-SPONSOR-DECISION-SNAPSHOT-V2"
SPONSOR_OBSERVATION_DECISION_V2_CONTRACT_ID = "KRONOS-SWING-SPONSOR-OBSERVATION-DECISION-V2"
SPONSOR_OBSERVATION_STORE_V2_SCHEMA = "KRONOS-SWING-SPONSOR-OBSERVATION-STORE-V2"
...
@dataclass(frozen=True, slots=True)
class SponsorDecisionSnapshotV2:
    snapshot_identity: str
    native_run_identity: str
    canonical_instrument: str
    native_assessment_sha256: str
    ...
    mcx_supporting_context_sha256: str | None
    snapshot_timestamp: datetime
    kr370_contract_version: str
    confirmation_state: str
    confirmation_integrity_sha256: str
    eligibility_handoff_identity: str
    eligibility_handoff_sha256: str
    integrity_sha256: str
    contract_identity: str = SPONSOR_DECISION_SNAPSHOT_V2_CONTRACT_ID
    ...
    def __post_init__(self) -> None:
        if (
            not _identity(self.snapshot_identity)
            or not self.native_run_identity or not self.canonical_instrument
            or not _digest(self.native_assessment_sha256)
            or self.direction not in {V1Direction.LONG, V1Direction.SHORT}
            or not self.kr370_identity.startswith("KRONOS-KR-370-SWING-ANALYTICAL-PROMOTION-V2:2:")
            or not _digest(self.kr370_integrity_sha256)
            or self.kr370_identity.rsplit(":", 1)[-1] != self.kr370_integrity_sha256
            or self.kr370_contract_version != "2"
            or self.kr370_state not in {"BUY_NOW", "SELL_NOW"}
            or self.kr370_hard_gate_state != "NO_HARD_GATE"
            or tuple(item[0] for item in self.kr370_criteria) != (
                "K1_1H_DIRECTIONAL_PROGRESSION", "K2_1H_CPR_ACCEPTANCE",
                "K3_IMMEDIATE_PATH_CLEARANCE", "K4_SETUP_QUALITY", "K5_NON_EXTENSION")
            or any(item[1] != "SATISFIED" for item in self.kr370_criteria)
            ...
            or not _identity(self.eligibility_handoff_identity)
            or not _digest(self.eligibility_handoff_sha256)
            or not _identity(self.visual_evidence_identity)
            or not _digest(self.visual_evidence_sha256)
            or not _identity(self.step31_observation_identity)
            or not _digest(self.step31_observation_sha256)
            ...
            or (self.conventional_trade_plan_identity is None)
            != (self.conventional_trade_plan_sha256 is None)
            or (self.conventional_trade_plan_identity is not None and
                (not _identity(self.conventional_trade_plan_identity)
                 or not _digest(self.conventional_trade_plan_sha256)))
            or self.risk_state not in {
                "RISK_APPROVED", "RISK_CONSTRAINED", "RISK_REJECTED", "RISK_UNAVAILABLE"}
            ...
            or (self.mcx_supporting_context_identity is None)
            != (self.mcx_supporting_context_sha256 is None)
            or (self.mcx_supporting_context_identity is not None and
                (not _identity(self.mcx_supporting_context_identity)
                 or not _digest(self.mcx_supporting_context_sha256)))
            or not _aware(self.snapshot_timestamp)
            or (self.contract_identity, self.contract_version) != (
                SPONSOR_DECISION_SNAPSHOT_V2_CONTRACT_ID, "2")
            or self.policy_identity != SPONSOR_OBSERVATION_POLICY_ID
            or self.policy_version != SPONSOR_OBSERVATION_POLICY_VERSION
            or self.authority != SPONSOR_OBSERVATION_AUTHORITY
            or self.integrity_sha256 != _record_digest(self)
        ):
            raise ValueError("SPONSOR_DECISION_SNAPSHOT_V2_INVALID")


@dataclass(frozen=True, slots=True)
class SponsorObservationDecisionV2:
    decision_identity: str
    snapshot_identity: str
    snapshot_sha256: str
    native_run_identity: str
    ...
    decision_timestamp: datetime
    integrity_sha256: str
    contract_identity: str = SPONSOR_OBSERVATION_DECISION_V2_CONTRACT_ID
    contract_version: str = "2"
    policy_identity: str = SPONSOR_OBSERVATION_POLICY_ID
    policy_version: str = SPONSOR_OBSERVATION_POLICY_VERSION
    authority: str = SPONSOR_OBSERVATION_AUTHORITY
    ...
    def __post_init__(self) -> None:
        if (
            not _identity(self.decision_identity) or not _identity(self.snapshot_identity)
            or not _digest(self.snapshot_sha256) or not self.native_run_identity
            or not self.canonical_instrument or not _digest(self.native_assessment_sha256)
            or self.direction not in {V1Direction.LONG, V1Direction.SHORT}
            or self.kr370_state not in {"BUY_NOW", "SELL_NOW"}
            or not self.kr370_record_identity.startswith(
                "KRONOS-KR-370-SWING-ANALYTICAL-PROMOTION-V2:2:")
            or not _digest(self.kr370_integrity_sha256)
            or self.kr370_record_identity.rsplit(":", 1)[-1] != self.kr370_integrity_sha256
            or not _identity(self.eligibility_handoff_identity)
            or not _digest(self.eligibility_handoff_sha256)
            or type(self.choice) is not SponsorTradeChoice
            or (self.sponsor_reason is not None
                and type(self.sponsor_reason) is not SponsorObservationReason)
            or type(self.warning_acknowledged) is not bool
            or not _aware(self.decision_timestamp)
            or (self.contract_identity, self.contract_version) != (
                SPONSOR_OBSERVATION_DECISION_V2_CONTRACT_ID, "2")
            or self.policy_identity != SPONSOR_OBSERVATION_POLICY_ID
            or self.policy_version != SPONSOR_OBSERVATION_POLICY_VERSION
            or self.authority != SPONSOR_OBSERVATION_AUTHORITY
            or self.integrity_sha256 != _record_digest(self)
        ):
            raise ValueError("SPONSOR_OBSERVATION_DECISION_V2_INVALID")
...
class SponsorObservationDecisionResult:
    snapshot: SponsorDecisionSnapshotV1 | SponsorDecisionSnapshotV2
    decision: SponsorObservationDecisionV1 | SponsorObservationDecisionV2
    activation: SponsorActivationDispositionV1

    def __post_init__(self) -> None:
        if type(self.snapshot) is SponsorDecisionSnapshotV2 and (
            type(self.decision) is not SponsorObservationDecisionV2
            or self.decision.snapshot_identity != self.snapshot.snapshot_identity
            or self.decision.snapshot_sha256 != self.snapshot.integrity_sha256
            or self.decision.kr370_record_identity != self.snapshot.kr370_identity
            or self.decision.kr370_integrity_sha256 != self.snapshot.kr370_integrity_sha256
            or self.decision.eligibility_handoff_identity != self.snapshot.eligibility_handoff_identity
            or self.decision.eligibility_handoff_sha256 != self.snapshot.eligibility_handoff_sha256
            or self.activation.decision_identity != self.decision.decision_identity
        ):
            raise ValueError("SPONSOR_OBSERVATION_V2_LINEAGE_INVALID")
        if type(self.decision) is SponsorObservationDecisionV2 and type(self.snapshot) is not SponsorDecisionSnapshotV2:
            raise ValueError("SPONSOR_OBSERVATION_V2_LINEAGE_INVALID")
...
def record_sponsor_observation_decision(
    promotion: Kr370AnalyticalPromotionRecord | V2PromotionRecord,
    ...
    *,
    ...
    eligibility_handoff: Kr370Step31EligibilityHandoffV2 | None = None,
) -> SponsorObservationDecisionResult:
    """Create immutable observation evidence after all currentness gates pass."""

    if type(promotion) is V2PromotionRecord:
        return _record_sponsor_observation_decision_v2(
            promotion, observation, handoff, choice, disposition,
            current_run_identity=current_run_identity, decided_at=decided_at,
            warning_acknowledged=warning_acknowledged, sponsor_reason=sponsor_reason,
            risk_identity=risk_identity, risk_state=risk_state,
            ...
            eligibility_handoff=eligibility_handoff)
    if (
...
def _record_sponsor_observation_decision_v2(
    ...
    *,
    ...
    mcx_supporting_context_identity: str | None,
    mcx_supporting_context_sha256: str | None,
    eligibility_handoff: Kr370Step31EligibilityHandoffV2 | None,
) -> SponsorObservationDecisionResult:
    value = promotion.value
    source, confirmation = value["source"], value["confirmation"]
    if (
        type(eligibility_handoff) is not Kr370Step31EligibilityHandoffV2
        or source["market"] != "NSE"
        or source["asset_class"] not in {"NSE_EQUITY", "NSE_INDEX"}
        ...
        or eligibility_handoff.kr370_record_identity != promotion.identity
        or eligibility_handoff.kr370_record_integrity_sha256 != value["integrity_sha256"]
        or eligibility_handoff.confirmation_state != confirmation["state"]
        or eligibility_handoff.confirmation_integrity_sha256 != sha256(_canonical(confirmation)).hexdigest()
        or eligibility_handoff.native_run_identity != source["native_run_identity"]
        or eligibility_handoff.canonical_instrument != source["canonical_instrument"]
        or eligibility_handoff.native_assessment_sha256 != source["native_assessment_sha256"]
        or current_run_identity != source["native_run_identity"]
        or observation.native_run_identity != source["native_run_identity"]
        or observation.canonical_instrument != source["canonical_instrument"]
        or observation.native_assessment_sha256 != source["native_assessment_sha256"]
        or observation.kr370_handoff_identity != eligibility_handoff.handoff_identity
        or observation.kr370_handoff_integrity_sha256 != eligibility_handoff.integrity_sha256
        or handoff.native_run_identity != source["native_run_identity"]
        or handoff.canonical_instrument != source["canonical_instrument"]
        or handoff.native_assessment_sha256 != source["native_assessment_sha256"]
        or handoff.kr370_handoff_identity != eligibility_handoff.handoff_identity
        or handoff.observation_evidence_id != observation.observation_evidence_id
        or handoff.observation_evidence_sha256 != observation.integrity_sha256
    ):
        raise ValueError("SPONSOR_OBSERVATION_V2_TRUST_BINDING_INVALID")
    if (not _aware(decided_at) or type(choice) is not SponsorTradeChoice
            or type(warning_acknowledged) is not bool
            or (sponsor_reason is not None and type(sponsor_reason) is not SponsorObservationReason)):
        raise ValueError("SPONSOR_OBSERVATION_REQUEST_INVALID")
    if (observation.severity is Step31WarningSeverity.RED
            and choice in {SponsorTradeChoice.PAPER, SponsorTradeChoice.LIVE}
            and warning_acknowledged is not True):
        raise ValueError("SPONSOR_OBSERVATION_WARNING_ACKNOWLEDGEMENT_REQUIRED")
    if choice is SponsorTradeChoice.IGNORE:
        ...
        SponsorActivationDisposition.ACTIVATED, SponsorActivationDisposition.BLOCKED_CONSTRAINT,
    }:
        raise ValueError("SPONSOR_OBSERVATION_ACTIVATION_DISPOSITION_INVALID")
    elif risk_state == "RISK_APPROVED" and observation.conventional_trade_plan_id is None and disposition is not SponsorActivationDisposition.BLOCKED_MISSING_VALID_PLAN:
        raise ValueError("SPONSOR_OBSERVATION_ACTIVATION_DISPOSITION_INVALID")
    if disposition is SponsorActivationDisposition.ACTIVATED and (
        ...
        raise ValueError("SPONSOR_OBSERVATION_ACTIVATION_DISPOSITION_INVALID")
    snapshot_identity = _id(
        "SPONSOR-DECISION-SNAPSHOT-V2", observation.integrity_sha256,
        value["integrity_sha256"], choice.value, decided_at.isoformat())
    snapshot_values = dict(
        snapshot_identity=snapshot_identity,
        native_run_identity=source["native_run_identity"],
        canonical_instrument=source["canonical_instrument"],
        native_assessment_sha256=source["native_assessment_sha256"],
        direction=V1Direction(source["direction"]),
        kr370_identity=promotion.identity,
        kr370_integrity_sha256=value["integrity_sha256"],
        kr370_state=value["promotion_state"],
        kr370_criteria=tuple((item["identity"], item["state"]) for item in value["criteria"]),
        kr370_hard_gate_state="NO_HARD_GATE",
        visual_evidence_identity=source["acceptance"]["review_pack_identity"],
        visual_evidence_sha256=sha256(_canonical(source["acceptance"]["visual_bindings"])).hexdigest(),
        step31_observation_identity=observation.observation_evidence_id,
        step31_observation_sha256=observation.integrity_sha256,
        step31_geometry_status=observation.geometry_status.value,
        step31_severity=observation.severity,
        step31_warnings=tuple(item.value for item in observation.warnings),
        entry=observation.entry, stop=observation.stop,
        target=observation.canonical_target,
        invalidation=observation.invalidation_reference,
        risk_distance=observation.risk_per_unit,
        reward_distance=observation.reward_per_unit,
        ...
        mcx_supporting_context_sha256=mcx_supporting_context_sha256,
        snapshot_timestamp=decided_at,
        kr370_contract_version="2",
        confirmation_state=confirmation["state"],
        confirmation_integrity_sha256=eligibility_handoff.confirmation_integrity_sha256,
        eligibility_handoff_identity=eligibility_handoff.handoff_identity,
        eligibility_handoff_sha256=eligibility_handoff.integrity_sha256,
        contract_identity=SPONSOR_DECISION_SNAPSHOT_V2_CONTRACT_ID,
        contract_version="2",
        policy_identity=SPONSOR_OBSERVATION_POLICY_ID,
        policy_version=SPONSOR_OBSERVATION_POLICY_VERSION,
        authority=SPONSOR_OBSERVATION_AUTHORITY,
        integrity_sha256="")
    snapshot = SponsorDecisionSnapshotV2(**(
        snapshot_values | {"integrity_sha256": _values_digest(snapshot_values)}))
    decision_identity = _id(
        "SPONSOR-OBSERVATION-DECISION-V2", snapshot_identity, choice.value)
    decision_values = dict(
        decision_identity=decision_identity, snapshot_identity=snapshot_identity,
        snapshot_sha256=snapshot.integrity_sha256,
        native_run_identity=source["native_run_identity"],
        canonical_instrument=source["canonical_instrument"],
        native_assessment_sha256=source["native_assessment_sha256"],
        ...
        eligibility_handoff_identity=eligibility_handoff.handoff_identity,
        eligibility_handoff_sha256=eligibility_handoff.integrity_sha256,
        choice=choice, sponsor_reason=sponsor_reason,
        warning_acknowledged=warning_acknowledged,
        decision_timestamp=decided_at,
        contract_identity=SPONSOR_OBSERVATION_DECISION_V2_CONTRACT_ID,
        contract_version="2",
        policy_identity=SPONSOR_OBSERVATION_POLICY_ID,
        policy_version=SPONSOR_OBSERVATION_POLICY_VERSION,
        authority=SPONSOR_OBSERVATION_AUTHORITY,
        integrity_sha256="")
    decision = SponsorObservationDecisionV2(**(
        decision_values | {"integrity_sha256": _values_digest(decision_values)}))
    activation_values = dict(
        disposition_identity=_id("SPONSOR-ACTIVATION-DISPOSITION",
                                 decision_identity, disposition.value),
        decision_identity=decision_identity, disposition=disposition,
        reason=disposition.value,
        existing_sponsor_decision_identity=existing_sponsor_decision_identity,
        sponsor_position_identity=sponsor_position_identity, recorded_at=decided_at,
        contract_identity=SPONSOR_ACTIVATION_DISPOSITION_CONTRACT_ID,
        contract_version="1",
        authority="FACTUAL_ACTIVATION_OUTCOME_ONLY_NO_ACTIVATION_AUTHORITY",
        integrity_sha256="")
...
class LocalSponsorObservationDecisionStore:
    ...
    def retain(self, result: SponsorObservationDecisionResult) -> SponsorObservationDecisionResult:
        ...
        version = "2" if type(result.snapshot) is SponsorDecisionSnapshotV2 else "1"
        path = self._path(result.snapshot.native_run_identity, result.snapshot.canonical_instrument,
                          version=version)
        payload = {"schema": (SPONSOR_OBSERVATION_STORE_V2_SCHEMA if version == "2"
                              else SPONSOR_OBSERVATION_STORE_SCHEMA),
                   "result": _primitive(result)}
        with self._lock:
            if path.exists():
    ...
    def transition_activation(
        ...
        with self._lock:
            ...
            payload = {
                "schema": (SPONSOR_OBSERVATION_STORE_V2_SCHEMA if version == "2"
                           else SPONSOR_OBSERVATION_STORE_SCHEMA),
    ...
    def load(self, path: Path) -> SponsorObservationDecisionResult:
        ...
        version = "2" if type(restored.snapshot) is SponsorDecisionSnapshotV2 else "1"
        activation_path = path.with_name("activation-v2.json" if version == "2" else "activation.json")
        if not activation_path.exists():
            return restored
        try:
            transition = json.loads(activation_path.read_text(encoding="utf-8"))
            if transition.get("schema") != (
                SPONSOR_OBSERVATION_STORE_V2_SCHEMA if version == "2"
                else SPONSOR_OBSERVATION_STORE_SCHEMA):
                raise ValueError
    ...
    def load_initial(self, path: Path) -> SponsorObservationDecisionResult:
        ...
        try:
            ...
            if schema not in {SPONSOR_OBSERVATION_STORE_SCHEMA,
                              SPONSOR_OBSERVATION_STORE_V2_SCHEMA}:
                raise ValueError
            result = payload["result"]
            v2 = schema == SPONSOR_OBSERVATION_STORE_V2_SCHEMA
            if (path.name == "decision-v2.json") != v2:
                raise ValueError
            return SponsorObservationDecisionResult(
                (_snapshot_v2_from_dict(result["snapshot"]) if v2
                 else _snapshot_from_dict(result["snapshot"])),
                (_decision_v2_from_dict(result["decision"]) if v2
                 else _decision_from_dict(result["decision"])),
    ...
    def for_current_observations(
        ...
        *, promotion_versions: dict[tuple[str, str], int] | None = None,
        ...
        for result in self.load_all():
            ...
            observation = expected.get(key)
            if observation is None:
                continue
            version = 2 if type(result.snapshot) is SponsorDecisionSnapshotV2 else 1
            selected_version = None if promotion_versions is None else promotion_versions.get(key)
            if selected_version is not None and version != selected_version:
                if (result.snapshot.step31_observation_identity == observation.observation_evidence_id
                        and result.snapshot.step31_observation_sha256 == observation.integrity_sha256):
                    raise ValueError("SPONSOR_OBSERVATION_RESTART_VERSION_MIXED")
                # A different-version observation remains historical and readable
                # through the explicit full-history store API.
                continue
            if (
    ...
    def _path(self, run_id: str, instrument: str, *, version: str = "1") -> Path:
        return self.root / run_id / instrument / (
            "decision-v2.json" if version == "2" else "decision.json")

    def _activation_path(self, run_id: str, instrument: str, *, version: str = "1") -> Path:
        return self.root / run_id / instrument / (
            "activation-v2.json" if version == "2" else "activation.json")
...
def _snapshot_v2_from_dict(value: dict[str, object]) -> SponsorDecisionSnapshotV2:
    data = dict(value)
    for name in ("entry", "stop", "target", "invalidation", "risk_distance",
    ...
    data["step31_warnings"] = tuple(data["step31_warnings"])
    data["snapshot_timestamp"] = datetime.fromisoformat(str(data["snapshot_timestamp"]))
    return SponsorDecisionSnapshotV2(**data)
...
def _decision_v2_from_dict(value: dict[str, object]) -> SponsorObservationDecisionV2:
    data = dict(value)
    data["direction"] = V1Direction(data["direction"])
    data["choice"] = SponsorTradeChoice(data["choice"])
    data["sponsor_reason"] = (None if data["sponsor_reason"] is None
                              else SponsorObservationReason(data["sponsor_reason"]))
    data["decision_timestamp"] = datetime.fromisoformat(str(data["decision_timestamp"]))
    return SponsorObservationDecisionV2(**data)

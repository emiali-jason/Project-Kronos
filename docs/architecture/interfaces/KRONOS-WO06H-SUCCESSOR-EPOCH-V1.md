...
## Deterministic same-epoch capability — contract 1.2.0

[ADR-0047](../adr/ADR-0047-WO06H-DETERMINISTIC-ACCEPTANCE-EPOCH-DIGEST.md) replaces the 1.1.0 marshal/code-object digest with `KRONOS-WO06H-ACCEPTANCE-EPOCH-SEMANTIC-AST/1.0.0`. The canonical payload binds exact policy/schema/invariant declarations, the documented explicit callable allowlist as position-free canonical AST, and the separate immutable `WO_06H_LIVE_SHADOW` calculation declaration. Startup-pinned source is authoritative. Paths, loader origins, source positions, code-object metadata and the interpreter-owned `contextlib` wrapper are excluded.

Historical 1.1.0 proofs remain unchanged. New immutable `compatibility` documents use `WO06H-COMPATIBILITY-<sha256>` identities. Their exact body keys are `epoch`, `acceptance`, `window`, `historical_proof`, `corrected_proof`, `diagnosis`, `classification`, `protected_sources`, `equivalence_evidence`, and `sponsor_authorization`. The linked collateral diagnosis binds the same epoch/acceptance/window/proof/source/evidence fields plus `report_sha256` and `sponsor_reference`; classification is exactly `DIGEST_SCOPE_COLLATERAL`.

This mechanism does not change the successor API and cannot be supplied by an HTTP caller. It permits only exact 1.1.0→1.2.0 restoration for the bound existing epoch when configuration, every other capability and the live-shadow calculation declaration are identical. Missing, duplicate, corrupt, wrong-epoch, wrong-proof, future-version or calculation/capability mismatch remains `SHADOW_RESTORATION_RUNTIME_INCOMPATIBLE` or a bounded compatibility validation failure. It creates no acceptance, window, epoch, pointer, observation or backfill.

## Same-epoch compatibility restoration surface — contract 1.0.0

[ADR-0048](../adr/ADR-0048-WO06H-SAME-EPOCH-COMPATIBILITY-RESTORATION.md) adds a separate application surface for an already-retained exact compatibility record. This does not change the successor API or its authorization gate.

`POST /control/intraday-live-shadow/compatibility-restoration/v1` is same-origin, maintenance-only, `application/json`, query-free and bounded to 65,536 bytes. Exact keys are `policy`, `action`, `epoch_identity`, `acceptance_identity`, `window_identity`, `current_runtime_proof`, `current_capability_proof`, `compatibility_identity`, `sponsor_authorization`, `request_identity`, and `integrity`. Policy is `KRONOS-WO06H-SAME-EPOCH-COMPATIBILITY-RESTORATION/1.0.0`; action is `RESTORE_SAME_EPOCH_COMPATIBILITY`. Request identity and integrity are deterministic hashes of the exact request. Unknown keys and all creation, migration or backfill parameters reject.

Server-owned state must establish active maintenance and complete quiescence. The application re-derives the runtime proof from the bound process and requires exact request equality, current PID ownership, `CLEAN_COMMIT`, the exact complete capability identity, and `WO_06H_ACCEPTANCE_EPOCH/1.2.0`. CURRENT, chain head, epoch, acceptance and window must all be exact and internally valid. The named compatibility and diagnosis records must validate and be the unique exact equivalence for that epoch and current proof. The Sponsor/EA reference in the request must equal the immutable compatibility record.

Success returns `RESTORED` or `ALREADY_RESTORED` with the exact existing epoch, acceptance, window, compatibility and request identities plus current status. Transport rejection is HTTP 409 with `SHADOW_COMPATIBILITY_RESTORATION_REJECTED`. Every rejection is inert. The operation invokes only existing restoration and creates no durable record. It does not validate or reuse the old successor authorization because that authorization governed creation.

...
| KR-260B | Market Data Retrieval Engine | Retrieves standardized market data from KR-260A routing outputs. | `0.1.0` | `FOUNDATION` | Foundation | Future KR-260A public routing outputs | Future retrieved datasets, availability flags, status, and reasons | Future KR-260 normalization | Contract/header foundation only. No retrieval logic or `request.security()` calls are implemented. |
